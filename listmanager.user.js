...
The election has now been called! We need people to hand out 'How to Vote' cards at polling booths across [electorate] ([suburb]) on election day (21st of March). If you are able to help I can roster you on at a time and place that suits.`;
    ...
    if (IS_LISTMANAGER) {
        ...
        function buildPdTree() {
            return JSON.stringify({
                op: 'intersection',
                nodes: [
                    { op: 'filter', filter: { name: 'roster', value: { electionId: 182, electorateIds: [], rosterTypes: ['Rostered', 'Self-rostered'], shiftStatus: 'Any', votingPeriod: 'Polling Day' } } }
                ],
                printTime: false,
        ...
        function buildEvTree() {
            return JSON.stringify({
                op: 'intersection',
                nodes: [
                    { op: 'filter', filter: { name: 'roster', value: { electionId: 182, electorateIds: [], rosterTypes: ['Rostered', 'Self-rostered'], shiftStatus: 'Any', votingPeriod: 'Early voting' } } }
                ],
                printTime: false,
                useAdvancedSearchII: false
            });
        ...
        function fetchRosterCount(callback) {
            ...
            fetchOneRoster(buildPdTree(), function(count, err, entities) {
                if (err === 'auth_expired') { authExpired = true; }
                else if (err) { rosterError = err; }
                else { pdTotal = count; _rosterEntities.pd = entities || []; }
                checkDone();
            });

            fetchOneRoster(buildEvTree(), function(count, err, entities) {
                if (err === 'auth_expired') { authExpired = true; }
                else if (err) { if (!rosterError) rosterError = err; }
                else { evTotal = count; _rosterEntities.ev = entities || []; }
                checkDone();
            });
        ...
        function openBreakdownPopup() {
                    ...
                    <div class="gus-breakdown-rings">
                        <div class="gus-breakdown-ring-wrap">
                            <div class="gus-roster-ring gus-breakdown-ring">
                                ...
                                <span class="gus-roster-pct" data-default="${(_grandTotal ?? ((pdTotal ?? 0) + (evTotal ?? 0))).toLocaleString()}">${(_grandTotal ?? ((pdTotal ?? 0) + (evTotal ?? 0))).toLocaleString()}</span>
                            </div>
            ...
            // Show progress ring immediately (pdTotal already known)
            updateTotalRing();
        ...
        function parseBooths(commands) {
            ...
            const edBooths = allBooths.filter(b => b.info.prepoll === '0').map(b => {
                ...
                return {
                    ...
                    address: [info.address1, info.address_suburb, info.address_state, info.postcode].filter(Boolean).join(', '),
        ...
        function computeElectorateSummary(booths) {
            const totalBooths = booths.filter(b => b.slotCoverage.some(sc => sc.need > 0)).length;
            const slotSummaries = BOOTH_TIME_SLOTS.map((_, si) => {
                let totalHave = 0, totalNeed = 0, cappedHave = 0;
                for (const b of booths) {
                    const sc = b.slotCoverage[si];
                    totalHave += sc.have;
                    totalNeed += sc.need;
                    cappedHave += Math.min(sc.have, sc.need);
                }
                return { totalHave, totalNeed, cappedHave, pct: totalNeed > 0 ? Math.round((cappedHave / totalNeed) * 100) : 100 };
            });
            const grandCapped = slotSummaries.reduce((s, v) => s + v.cappedHave, 0);
            const grandNeed = slotSummaries.reduce((s, v) => s + v.totalNeed, 0);
            const overallPct = grandNeed > 0 ? Math.round((grandCapped / grandNeed) * 100) : 100;
        ...
        function bcExportOverview(results) {
            ...
            if (filtered.length > 1) {
                ...
                const totalCells = [
                    { text: 'All Electorates', bold: true },
                    { text: String(grandBooths), align: 'center', bold: true }
                ];
                for (let si = 0; si < BOOTH_TIME_SLOTS.length; si++) {
                    const gs = grandSlots[si];
                    totalCells.push({ text: `${gs.have}/${gs.need}`, badge: true, colors: bcSlotColors(gs.capped, gs.need) });
                }
        ...
        function bcSearchResults(results, query) {
            ...
            for (const r of results) {
                ...
                if (matchingBooths.length > 0) {
                    filtered.push({ ...r, booths: matchingBooths, summary: computeElectorateSummary(matchingBooths) });
                }
            }
        ...
        function renderCoverageTable(results, tableEl, statusEl) {
            ...
            if (filtered.length > 1) {
                ...
                for (let si = 0; si < BOOTH_TIME_SLOTS.length; si++) {
                    const gs = grandSlots[si];
                    const cls = bcSlotClass(gs.capped, gs.need);
                    html += `<td><span class="gus-bc-slot ${cls}">${gs.have}/${gs.need}</span></td>`;
                }
                html += `<td class="gus-bc-pct ${bcPctClass(grandPct)}">${grandPct}%</td>`;
                html += '</tr></tfoot>';
            }
        ...
        function updateRosterWidget() {
            ...
            if (rosterLoading) {
                const all = [pdTotal, pdHeysen, pdCaptains, evTotal, evHeysen, evCaptains];
                const done = all.filter(v => v !== null).length;
                body.innerHTML = `<span class="gus-roster-loading" style="display:flex;align-items:center;gap:8px;"><span class="gus-spinner"></span><span style="font-size:12px;color:#999;">Loading ${done}/6</span></span>`;
                return;
            }
            if (rosterError) {
        ...
        let rosterJwtCheckInterval = setInterval(() => {
            ...
            if (capturedJwt && pdTotal === null && !rosterLoading) {
                fetchRosterCount();
            }
            rosterJwtCheckCount++;
            if (pdTotal !== null || rosterJwtCheckCount > 30) {
    ...
    if (IS_ROCKET) {
        ...
        const CONFIRMATION_TEMPLATE_BODY = `Hi [their name], a friendly reminder that you're on a booth tomorrow at [shifts]. A Greens t-shirt (if you requested one), How-to-Vote cards and great vibes will be there for you. Don't forget to vote yourself too!\n\nDouble your impact \u2014 other parties are out in force and we need to make sure voters see the Greens everywhere. If you know someone who could spare even an hour on a booth tomorrow, send me their name and number (with consent) or share mine: 0434 331 085 ([your name], Greens volunteer coordinator). Thank you!`;
